...
def text_to_svg(lines):
    ...
    # Load current Y position (circular buffer)
    y = load_state()
    final_y = y
    ...
    # Save the next Y position for next run
    save_state(final_y)

    dwg.add(main_g)
    dwg.save()
    print(f"Saved A4 SVG as: {OUTPUT_SVG}")
    print(f"Next line will start at Y: {final_y}")
    
    # Open in Inkscape (non-blocking)
    subprocess.Popen(["inkscape", OUTPUT_SVG], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


if __name__ == "__main__":
    def get_user_lines():
        print("Enter lines of text. Submit an empty line to finish.")
        print("Press Enter immediately to use generated sample lines.")
        user_lines = []
        while True:
            try:
                s = input("> ").rstrip("\n")
            ...
            user_lines.append(s)
        if not user_lines:
            return [random_line() for _ in range(5)]
        return user_lines
